...
import { SEO } from '../SEO';
import { ArrowLeft, HelpCircle, ShieldAlert, Activity } from 'lucide-react';
        ...
        <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16 space-y-20">
          ...
          <section className="bg-card border border-border rounded-2xl p-8 lg:p-12 space-y-4">
            ...
            <p className="text-muted-foreground text-sm leading-relaxed">
              When evaluating a degradation trend inside an active evaporator effect, our technical teams calculate the localized fouling resistance index ($R_f$) to isolate hardware issues from operational parameters:
            </p>
            <div className="my-4 p-4 bg-muted/30 rounded-lg text-center font-mono text-sm border border-border text-primary">
              R<sub>f</sub> = 1 / U<sub>dirty</sub> - 1 / U<sub>clean</sub>
            </div>
            <p className="text-muted-foreground text-sm leading-relaxed">
          ...
          <section className="space-y-6">
            ...
            <div className="grid gap-4">
              {[
                ...
                { q: "How can a plant determine if low evaporation capacity is caused by tube scaling or short steam supply?", a: "We calculate the dynamic $U$-value across each effect. A high temperature difference ($\Delta T$) coupled with a declining $U$-value points directly to tube-side scaling rather than steam input starvation." },
                { q: "What diagnostic approach isolates non-condensable gas (NCG) pockets within a calendria shell?", a: "We trace surface temperature variations across the shell body. Unusually cool zones indicate NCG pockets, which can be cleared by updating venting orifice locations." },
                ...
                { q: "Can a field audit identify scaling causes without opening the evaporator bodies?", a: "Yes. By analyzing temperature profiles, feed concentration indices, and pressure changes ($\Delta P$) across specific loops over consecutive shifts, we can infer scaling trends." }
              ].map((faq, idx) => (
