        ...
        <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16 space-y-20">
            ...
            <div className="grid md:grid-cols-2 gap-8">
              <div className="bg-card p-8 rounded-2xl border border-border space-y-4">
                ...
                <p className="text-sm text-muted-foreground leading-relaxed">
                  Operates by feeding dilute liquor through custom distribution trays at the upper tube sheet. The liquid forms a thin, highly turbulent film that flows downward under gravity along the inner walls of the $31.75\text{'mm'}$ OD tubes. This layout yields maximum heat transfer coefficients ($U$-values of $1800 - 2400 \text{ 'W/m'}^2\cdot\text{'K'}$) under low temperature differences ($\Delta T$).
                </p>
              </div>
              <div className="bg-card p-8 rounded-2xl border border-border space-y-4">
                <h3 className="text-xl font-bold text-primary">Forced Circulation Evaporators (FCE)</h3>
                <p className="text-sm text-muted-foreground leading-relaxed">
                  Engineered specifically for heavy crystallizing arrays or thick non-Newtonian liquids. High-capacity axial pumps drive fluid through the tube core at high linear velocities ($2.0 \text{ 'm/s'}$ to $2.5 \text{ 'm/s'}$). Hydrostatic head suppresses boiling inside the tubes, forcing flash evaporation and solid crystal dropout to happen strictly inside the separator vessel.
                </p>
              </div>
            </div>
          </section>
          ...
          <section className="bg-card border border-border rounded-2xl p-8 lg:p-12 space-y-4">
            ...
            <p className="text-muted-foreground text-sm leading-relaxed">
              The boiling point elevation ($\text{'BPE'}'$) climbs as a solution grows more concentrated. If this elevation is undercalculated during system design, the effective temperature driving force collapses entirely:
            </p>
           <div className="my-4 p-4 bg-muted/30 rounded-lg text-center font-mono text-sm border border-border text-primary">
  ΔT<sub>eff</sub> = T<sub>heating steam</sub> - T<sub>vapor</sub> - ∑BPE
</div>
            <p className="text-muted-foreground text-sm leading-relaxed">
              Thermovap prevents system stalls by tailoring layout density. We utilize calculated triangular tube pitches ($60^\circ$ orientation) paired with exact wetting rates to prevent local dry spots, managing salt formation and maximizing continuous operation runtime.
            </p>
          </section>
          ...
          <section className="space-y-6">
            ...
            <div className="grid gap-4">
              {[
                ...
                { q: "Why is a $31.75\text{mm}$ OD with $1.2\text{mm}$ thickness used as the tube baseline?", a: "This configuration provides a balanced area-to-volume ratio for optimized heat flux while maintaining the structural strength needed to prevent collapsing under full vacuum conditions." },
                { q: "How does the layout arrangement affect heat transfer efficiency?", a: "A $60^\circ$ triangular pitch packs up to 15% more heat transfer area inside the same shell diameter compared to a square pitch layout, maximizing thermal output." },
