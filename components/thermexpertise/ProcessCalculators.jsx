        ...
        <div>
          ...
          <section className="space-y-6">
            <h2 className="text-2xl font-bold text-card-foreground tracking-tight border-l-4 border-primary pl-3">
              Process Calculations FAQ Matrix
            </h2>
            <div className="grid gap-4">
              {[
                { q: "What specific calculations are included in a Thermovap design support package?", a: "We deliver full thermal mass and energy balances, accurate heat transfer area (A) sizing, Log Mean Temperature Difference (LMTD) metrics, tube bundle counts matching your pass requirements, and shell thickness checks calculated to meet standard safety codes." },
                { q: "Why is calculating the exact Boiling Point Elevation (BPE) critical for MEE sizing?", a: "BPE raises the real boiling point of a solution as concentration levels grow. If this isn't precisely modeled, the effective temperature driving force (Delta-T) collapses, causing downstream evaporator effects to stall completely." },
                ...
                { q: "How is the overall heat transfer coefficient (U) verified during concentration transitions?", a: "We run iterative transport property models across incremental dry-solids boundaries, recalculating local film resistance changes to evaluate exact U-value drops as fluids thick down." },
                { q: "What mathematical logic regulates the selection of a specific tube pitch geometry?", a: "We balance the structural pitch ratio limits against fluid bypass tendencies and tube sheet ligament safety bounds, ensuring optimal thermal density without compromising tube bundle mechanical strength." },
                { q: "How do you calculate the required vapor velocity constraints to eliminate liquid droplet carryover?", a: "We model the terminal settling velocity of the liquid particles using droplet force balances, sizing the upper separator drum cross-sectional limits so that the upward vapor escape speed remains safely below the fluid entrainment boundaries." }
              ].map((faq, idx) => (
